...
# ===========================
# CRLB (kept for reference, not plotted here)
# ===========================
...
for i_beta, beta in enumerate(missing_fractions):
    ...
    v_missing = np.array(v_missing)
    v_lin = np.array(v_lin)
    v_sim = np.array(v_sim)
    v_kf = np.array(v_kf)
    v_rts = np.array(v_rts)
    v_bayes = np.array(v_bayes)
    v_pgmi_bayes = np.array(v_pgmi_bayes)

    # Store MAE
    phi_mae["missing_data"].append(np.mean(np.abs(v_missing - phi_true)))
    phi_mae["linear"].append(np.mean(np.abs(v_lin - phi_true)))
    phi_mae["sim"].append(np.mean(np.abs(v_sim - phi_true)))
    phi_mae["kalman_filt"].append(np.mean(np.abs(v_kf - phi_true)))
    phi_mae["kalman_smooth"].append(np.mean(np.abs(v_rts - phi_true)))
    phi_mae["bayes"].append(np.mean(np.abs(v_bayes - phi_true)))
    phi_mae["pgmi_bayes"].append(np.mean(np.abs(v_pgmi_bayes - phi_true)))

    # Store MSE
    phi_mse["missing_data"].append(np.mean((v_missing - phi_true)**2))
    phi_mse["linear"].append(np.mean((v_lin - phi_true)**2))
    phi_mse["sim"].append(np.mean((v_sim - phi_true)**2))
    phi_mse["kalman_filt"].append(np.mean((v_kf - phi_true)**2))
    phi_mse["kalman_smooth"].append(np.mean((v_rts - phi_true)**2))
    phi_mse["bayes"].append(np.mean((v_bayes - phi_true)**2))
    phi_mse["pgmi_bayes"].append(np.mean((v_pgmi_bayes - phi_true)**2))
...
for k in phi_mse:
    phi_mse[k] = np.asarray(phi_mse[k])
...
plt.plot(missing_fractions, phi_mae["missing_data"], label="Observed Only (data)", marker='o')
...
plt.plot(missing_fractions, phi_mae["kalman_filt"], label="Kalman Filter", marker='x')
...
plt.legend(ncol=2)
plt.tight_layout()
plt.savefig("phi_mae_plot_with_pgmi_bayesian.pdf", format="pdf", bbox_inches="tight")
plt.show()

# ===========================
# MSE table (Pandas DataFrame)
# ===========================
methods = [
    "missing_data", "linear", "sim",
    "kalman_filt", "kalman_smooth",
...
print("\n===== MSE TABLE (Phase Estimation) =====\n")
print(df_mse.round(6))

# Save to CSV
df_mse.to_csv("phi_mse_results.csv", index=False)
