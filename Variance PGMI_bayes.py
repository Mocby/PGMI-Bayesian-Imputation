...
# ===========================
# Storage (LOCF and PGMI removed)
# ===========================
phi_variance = {
    "missing_data": [], "linear": [], "sim": [],
    "kalman_filt": [], "kalman_smooth": [], "bayes": [],
    "pgmi_bayes": []
}
...
for i_beta, beta in enumerate(missing_fractions):
    ...
    for mc in range(n_iterations):
        ...
        # 7) PGMI–Bayes (shrinkage)
        #    Build PGMI-like completed dataset inline (no separate PGMI method stored)
    ...
    # Store variances (ddof = 1)
    phi_variance["missing_data"].append(np.var(v_missing, ddof=1))
    phi_variance["linear"].append(np.var(v_lin, ddof=1))
    phi_variance["sim"].append(np.var(v_sim, ddof=1))
    phi_variance["kalman_filt"].append(np.var(v_kf, ddof=1))
    phi_variance["kalman_smooth"].append(np.var(v_rts, ddof=1))
    phi_variance["bayes"].append(np.var(v_bayes, ddof=1))
    phi_variance["pgmi_bayes"].append(np.var(v_pgmi_bayes, ddof=1))

# Convert lists to arrays
for k in phi_variance:
    phi_variance[k] = np.asarray(phi_variance[k])
...
plt.plot(missing_fractions, phi_variance["missing_data"], label="Observed Only", marker='o')
plt.plot(missing_fractions, phi_variance["linear"], label="Linear", marker='^')
#plt.plot(missing_fractions, phi_variance["sim"], label="Simulated", marker='d')
plt.plot(missing_fractions, phi_variance["kalman_filt"], label="Kalman", marker='x')
#plt.plot(missing_fractions, phi_variance["kalman_smooth"], label="RTS Smoother", marker='P')
plt.plot(missing_fractions, phi_variance["bayes"], label="Bayesian Imputation", marker='h')
plt.plot(missing_fractions, phi_variance["pgmi_bayes"],
         label="PGMI–Bayesian", marker='v')

plt.plot(missing_fractions, ideal_crlb_list, 'b--', label="CRLB (No Missing)")
plt.plot(missing_fractions, mcar_crlb_list, 'g--', label="MCAR CRLB")

plt.xlabel(r"Missing Data Fraction")
plt.ylabel(r"Variance of Estimated Phase $\phi$ (rad$^2$)")
plt.yscale("log")
...
plt.tight_layout()
sns.set_style("darkgrid")
plt.savefig("phi_variance_plot_with_pgmi_bayes.pdf", format="pdf", bbox_inches="tight")
plt.show()
